...
for i in range(int(input_signal_length)):
    ...
    if (costas_loop_carrier_lock_flag == 1) and (preamble_lock_flag == 0):
        # We are waiting for the increase (rise) in the signal level
        # Нарастание уровня сигнала, передний фронт
        # Текщее значение больше предыдущего
        if (abs(bit_signal_average_value) > bit_signal_level_threshold) and (abs(bit_signal_average_value) > abs(bit_signal_average_previous)):
            bit_signal_average_value_rising_flag = 1
        else:
            # If there was an increase in the signal level, and now a decline, then this was a maximum (peak)
            # This is the best moment to detect beats
            # Если было нарастание, а теперь нет, то это максимум
            # Это середина бита для приёмника и конец для передатчика т.к. фильтра даст максимум в конце
            if bit_signal_average_value_rising_flag == 1:
                bit_signal_average_value_rising_flag = 0
        ...
        if bit_digital_previous_value != bit_digital_value:
            ...
            if bit_value_change_counter > bit_value_change_lock_threshold:
                ...
                bit_digital_average_buffer_counter_value = costas_loop_average_buffer_counter

    # If the carrier is captured and the preamble, then we wait for the sync word, and then the data
    # Если несущая захвачена и преамбула, то ждём синхрослово, а потом данные
...
plt.plot(costas_loop_i_filter_output_debug, "-b", costas_loop_q_filter_output_debug, "-g", costas_loop_iq_filter_output_debug, "-r", bit_signal_average_value_debug, "-y")
